...
de unidades vendidas y el monto total. }
  ...
  Begin
    ...
    then begin
           new(a);
           ArmarProducto (p, elem); // aca podria ser de registro - registro o tambien registro - registro para agregar a lista o vector
           a^.dato:= p;
           a^.HI:= nil; 
           a^.HD:= nil;
         end                // Hasta aca entiendo bien lo que hace el codigo
    else if (elem.codigoProducto = a^.dato.codigo) // Si la siguiente venta es del mismo producto la actualizo
         then begin
              ...
              end           // Aca creo que solo pregunta si la siguiente venta (LeerVenta) ingresada es el mismo producto que el anterior, si es asi suma la cantidad total de unidades y el monto total.
							// Las ventas se ingresan por codigo de producto ? Porque solo se actualizaria si la siguiente venta es del mismo codigo de producto *******
							
         else if (elem.codigoProducto < a^.dato.codigo) // Entonces, si no es una venta del mismo producto voy a insertar en el arbol a la izq o der segun esta condicion
			  then InsertarElemento(a^.HI, elem) // si el elem es menor al codigo que ingrese anteriormente en el arbol, voy al lado izquierdo
              else InsertarElemento(a^.HD, elem); // si el elem es mayor al codigo que se ingresa anteriormente en el arbol, voy al lado derecho
