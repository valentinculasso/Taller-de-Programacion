...
procedure ImprimirVectorRecursivo (v: vector; dimL: integer);
	...
	procedure ImprimirVector(v: vector; dimL: integer; var i: integer);
	begin
		if(i < dimL)then begin
			i:= i + 1;
			writeln(v[i]);
			ImprimirVector(v, dimL, i);
		end;
	end;
var
	indice: integer;
...
Begin
  ...
  if (caracter <> '.' )  
  then begin
         CargarLista (l); // Porque esta aca y no al final ??
         new (nuevo);
         nuevo^.dato:= caracter;
         nuevo^.sig:= l;
         l:= nuevo;
